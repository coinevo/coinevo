...
Currently only Tor and I2P have been integrated into Coinevo. The usage of
...
`--tx-proxy` which tells `coinevod` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--tx-proxy tor,127.0.0.1:9050,10`
> `--tx-proxy i2p,127.0.0.1:9000`

which tells `coinevod` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `coinevod` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:28083,127.0.0.1:28083,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `coinevod` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `coinevod`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `coinevod` localhost port 30000.
...
`coinevod` RPC port - which is independent from the configuration for incoming
...
> HiddenServiceDir /var/lib/tor/data/coinevo
...
This will store key information in `/var/lib/tor/data/coinevo` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/coinevo/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Coinevo network should have
similar timestamps.
...
### Intermittent Coinevo Syncing

If a user only runs `coinevod` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `coinevod` as often as possible to conceal when transactions are being sent.
...
`coinevod` currently selects two outgoing connections every 5 minutes for
...
`coinevod` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `coinevod` should likely disconnect
